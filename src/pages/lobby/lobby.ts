...
export class LobbyPage {
  ...
  ionViewDidEnter() {
    this.api.ready.then(() => {
      var promises = [this.api.load("users"), this.api.load("visitors"), this.api.load("workers")];
  ...
  searchPerson() {
    ...
    if (!this.person) this.searchUser(query);
    if (!this.person) this.searchWorker(query);
  ...
  uploadImage(id) {
    return this.api
      .post("images/upload/visitor/" + id, { image: this.person.image_url })
      .then((data: any) => {
        console.log(data);
  ...
  getVisits() {
    this.api
      .get("visits?with[]=residence&with[]=user&with[]=vehicle&with[]=visitor&withCount[]=visitors" + this.append())
      .then((data: any) => {
        console.log(data);
  ...
  append() {
    var append = "&append[]=guest&order[id]=desc&paginate=100";
    return append;
  }
  ...
  actions_visit(visit, i) {
    ...
    if (visit.visitor)
      buttons.push({
        ...
        icon: "contact",
        handler: () => {
          this.editVisitor(visit.visitor);
        }
      });
    ...
    this.actionsheet
      .create({
        title:
          this.api.trans("literals.visit") +
          " " +
          this.api.trans("__.from") +
          " " +
        ...
        buttons: [
          {
            ...
            handler: (data) => {
              if (data && data.note) {
                this.api
                  .put("visits/" + visit.id, { note: data.note })
                  .then((resp) => {
  ...
  // Visitors
  getVisitors() {
    if (this.query == "") return (this.visitors = this.api.objects.visitors.slice(0, 50));
    ...
    return (this.visitors = this.api.objects.visitors
      .filter((visitor) => {
        ...
        if (
          ...
          this.api.residences_collection[visitor.residence_id].name.toLowerCase().indexOf(this.query.toLowerCase()) > -1
        )
          return true;
        return false;
      })
  ...
  visitorModal(visitor = null) {
    ...
    var modal = this.modal.create(
      VisitorPage,
      { visitor: visitor, residence: residence },
