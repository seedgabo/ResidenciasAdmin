...
export class VisitorPage {
  ...
  constructor(
    ...
    if (visitor) this.visitor = visitor;
  ...
  dismiss() {
    if (this.action == "update") this.visitor = this.navParams.get("visitor");
  ...
  loadParkings() {
    this.api
      .get("parkings?where[status]=available&limit=500")
      .then((parkings: any) => {
